...
threshold = 20; %soglia per convertire in bianco e nero
tolleranza = 5; %tolleranza per il confronto
colonna_black_pixel = 0; % colonne con numero di black pixel maggiore di 15
contatore_occhi_chiusi = 0; % numero di volte che gli occhi rimango chiusi per più di 5 secondi
...
if ~ isempty(box_viso)
    ...
    for i=1:size(box_viso, 1) 
        rectangle('position', box_viso(biggest_box_viso, :), 'lineWidth', 2, 'edgeColor', 'g');
    end

    %%% RITAGLIO IL VISO

    viso_ritagliato = imcrop(img, box_viso(biggest_box_viso,:));
    ...
    if ~ isempty(box_occhi)
        ...
        %%% DISEGNARE UN CERCHIO VERDE SULLA PUPILLA
        ...
        white_pixel = 0;
                for g = 1:size(immagine_filtrata,1)
                    for f = 1:size(immagine_filtrata,2)
                        if immagine_filtrata(g,f) == 0
        ...
        %%% CENT INDICA IL NUMERO DI OCCHI TROVATI --> SE UGUALE A ZERO
        %%% IMPLICA CHE NESSUN CERCHIO E' STATO RISCONTRATO --> OCCHI CHIUSI

        
        if numero_occhi == 0 
            disp('Chiusi')
            subplot(2,3,6),imshow(chiusi),title("Stato degli occhi")
            contatore_occhi_chiusi = contatore_occhi_chiusi + 1;
        if contatore_occhi_chiusi > 5
            disp('Stato di SONNOLENZA rilevato');
            subplot(2,3,6),imshow('segnaletica/sonnolenza.jpg'),title("Rilevamento") 
        end

 
        elseif colonna_black_pixel > threshold + tolleranza
            disp('Aperti');
            subplot(2,3,6),imshow(aperti),title("Stato degli occhi") 
            contatore_occhi_chiusi = 0;
        end
